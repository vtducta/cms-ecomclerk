...
class Product extends \Eloquent
{
    ...
    protected $fillable = ['title', 'cost', 'amazon_title', 'brand', 'asin',
    ...
    public function calculate()
    {
        ...
        $patterns = [
            '/packs? of ([0-9]+)/i',
            '/(\d+)?.*?(pack|packs|pk|packets)/i',
            '/^.*?(\d+)[\s]?[\-]?(pack|packs|pk|packets)/i'
        ];
        ...
        /*$this->pack_cost = $this->number_of_packs * $this->cost;
        $this->gross_profit_fba =  $this->net_after_fba - $this->pack_cost;
        $this->gross_roi  = @(($this->gross_profit_fba / $this->pack_cost) * 100);*/
        $this->save();
        return $this;
    }
